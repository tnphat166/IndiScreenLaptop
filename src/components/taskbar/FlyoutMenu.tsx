...
export const FlyoutMenu: React.FC<FlyoutMenuProps> = ({ icon, items, direction = 'up' }) => {
  ...
  const getMenuClasses = () => {
    let base = "absolute flex transition-all duration-300 ease-[cubic-bezier(0.25,0.46,0.45,0.94)] z-50 ";
    if (!isOpen) {
    ...
    switch (direction) {
      case 'up':
        base += "bottom-full left-1/2 -translate-x-1/2 mb-2 flex-col-reverse gap-2 ";
        if (!isOpen) base += "translate-y-4";
        break;
      case 'down':
        base += "top-full left-1/2 -translate-x-1/2 mt-2 flex-col gap-2 ";
        if (!isOpen) base += "-translate-y-4";
        break;
      case 'left':
        base += "right-full top-1/2 -translate-y-1/2 mr-2 flex-row-reverse gap-2 ";
        if (!isOpen) base += "translate-x-4";
        break;
      case 'right':
        base += "left-full top-1/2 -translate-y-1/2 ml-2 flex-row gap-2 ";
        if (!isOpen) base += "-translate-x-4";
        break;
    }
    return base;
